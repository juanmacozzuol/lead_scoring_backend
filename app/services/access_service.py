...
def get_current_user(
    ...
    try:
        ...
        ## El token trae el 'nombre_usuario' en el campo 'sub', no el ID.
        ## Cambiamos la variable para que espere un string (str) y no un entero (int).
        username: str = payload.get("sub") # 'sub' es el estándar para el sujeto
        
        if username is None:
            raise credentials_exception
            
    except JWTError:
        # Esto atrapa errores de decodificación, firma inválida o expiración
        raise credentials_exception

    ## Buscamos al usuario por 'nombre_usuario'.
